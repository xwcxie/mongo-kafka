...
public class MongoDbShardUpdate extends MongoDbUpdate {
  ...
  public MongoDbShardUpdate(final MongoSinkTopicConfig config) {
    this.config = config;
  }
  ...
  public WriteModel<BsonDocument> perform(final SinkDocument doc) {
    ...
    try {
      ...
      if (keys != null) {
        ...
        keyList.forEach(
            e -> {
              BsonValue bsonValue = updateDoc.get(e);
              if (bsonValue != null) {
                filterDoc.put(e, bsonValue);
              }
            });
      }
